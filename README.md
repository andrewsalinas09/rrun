...
**Version 2.3.3** (core v2.3.3, ps shim v1.7, bash shim v1.6) · Windows + WSL + any ssh-reachable host
...
- **Exit 125** means the transport itself failed to decode the payload on the target (e.g. no `base64` there) — it is never a payload exit code. Multi-hop intermediate hosts additionally need a POSIX-compatible login shell (final targets are wrapped in `sh -c`, so csh/fish login shells are fine there).
